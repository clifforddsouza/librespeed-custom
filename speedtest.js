...
this.worker.onmessage = function(e) {
    ...
    console.log("🛠 DEBUG: worker.onmessage received data", data);
    ...
    if (data.testState >= 4) { // 🔥 TEST COMPLETED
        clearInterval(this.updater);
        this._state = 4;

        console.log("✅ Test finished! Keeping final values:", data);

        // 🔥 Ensure elements exist before updating
        let updateText = (id, value) => {
            let el = document.getElementById(id);
            if (el) el.innerHTML = value;
        };

        updateText("ping", data.pingStatus || "N/A");
        updateText("jitter", data.jitterStatus || "N/A");
//	updateText("packetLoss", data.packetLossDisplay || "N/A");
      updateText("packetLoss", data.packetLossStatus || "N/A");
        updateText("packetsSent", data.packetsSent || "0");
	updateText("packetsReceived", data.packetsReceived || "0");
	updateText("packetsLost", data.packetsLost || "0");
	updateText("dlSpeed", data.dlStatus || "N/A");
        updateText("ulSpeed", data.ulStatus || "N/A");
